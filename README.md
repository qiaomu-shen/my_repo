...
- The jump expert uses a `READY -> CANDIDATE/TAKEOFF -> FLIGHT -> LANDING -> SETTLING` state machine. A vertical-only session emits at confirmed takeoff for low latency; directional sessions wait briefly for flight evidence and lateral impulse. Landing can never emit a second trigger.
...
The current multi-action APK is generated by CI as `apk/phone-motion-dino-controller-v0.2.1-debug.apk`.
...
- Vertical-only mode requires two takeoff samples. Directional mode additionally requires three low-magnitude flight samples.
